 ...
 * 
 * Node.js Runtime 필요: fs, path 모듈을 사용하는 recipe loader들을 사용합니다.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`💾 ${planRecords.length}개 식단 레코드 UPSERT 시도...`);
    console.log("💾 저장할 데이터 샘플:", JSON.stringify(planRecords[0], null, 2));

    // UPSERT 패턴 사용: UNIQUE 제약조건을 활용하여 기존 레코드 업데이트 또는 새로 삽입
    const { error: insertError, data: insertedData } = await supabase
      .from("diet_plans")
      .upsert(planRecords, {
        onConflict: "user_id,plan_date,meal_type",
        ignoreDuplicates: false, // 기존 레코드를 업데이트
      })
      .select();
