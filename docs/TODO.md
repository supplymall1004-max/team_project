...
- [ ] **Phase 9: 생애주기별 네온 알림 시스템 - 전체 생애주기 이벤트 확장**
  ...
  - [ ] 라이프스타일 이벤트 구현
    - [ ] 반려동물 입양 및 관리 알림 시스템
  - [ ] 반려동물 생애주기별 건강 관리 시스템 구현
    - [ ] 반려동물 건강 관리 데이터베이스 스키마 생성
      - [ ] `pets` 테이블 생성 (반려동물 프로필)
      - [ ] `pet_vaccination_records` 테이블 생성 (백신 기록)
      - [ ] `pet_weight_records` 테이블 생성 (체중 기록)
      - [ ] `pet_health_checkup_records` 테이블 생성 (건강 검진 기록)
      - [ ] `pet_vaccine_master` 테이블 생성 (백신 마스터 데이터)
    - [ ] 반려동물 생애주기 계산 로직 구현
      - [ ] AVMA/AAHA 기준 생애주기 단계 판별 (강아지: Puppy, Junior, Adult, Mature Adult, Senior, Geriatric / 고양이: Kitten, Junior, Adult, Mature Adult, Senior, Geriatric)
      - [ ] 실시간 나이 계산 (일/월/년 단위)
      - [ ] 생애주기별 건강 이벤트 자동 매칭
    - [ ] 반려동물 백신 추적 시스템 구현
      - [ ] D-Day 카운트다운 (백신 만료일로부터 2주 전 알람)
      - [ ] 다음 접종일 자동 계산 (접종 완료일 입력 시)
      - [ ] 종류별 백신 필터링 (강아지/고양이별)
      - [ ] 백신 일정 캘린더 뷰
      - [ ] 접종 완료/예정 백신 리스트
    - [ ] 반려동물 체중 추적 시스템 구현
      - [ ] 체중 기록 및 추적
      - [ ] 체중 변화 꺾은선 그래프 시각화
      - [ ] 분기별(3개월마다) 체중 기록 권고 알림
      - [ ] 이상 체중 범위 검사 및 경고
    - [ ] 반려동물 건강 검진 추적 시스템 구현
      - [ ] 구강 관리 (3세 이후 매년 치과 검진 및 스케일링 권고)
      - [ ] 혈액 검사 리마인더 (강아지 7세 이상 매년, 10세 이상 반기별 / 고양이 10세 이상 매년, 15세 이상 반기별)
      - [ ] 정기 검진 일정 관리 (성견/성묘 연 1회, 노령견/노묘 반기별)
      - [ ] 검진 결과 기록 및 추적
    - [ ] 반려동물 생애주기별 건강 이벤트 알림 구현
      - [ ] 중성화 수술 시기 안내 (강아지 6개월 전후, 고양이 5-6개월)
      - [ ] 생애주기별 맞춤 건강 이벤트 자동 매칭
    - [ ] 반려동물 프로필 관리 UI 구현
      - [ ] 반려동물 목록 페이지 (`/health/pets`)
      - [ ] 반려동물 상세 페이지 (`/health/pets/[petId]`)
      - [ ] 반려동물 등록 페이지 (`/health/pets/new`)
      - [ ] 반려동물 프로필 카드 컴포넌트
      - [ ] 건강 상태 요약 대시보드
    - [ ] LocalStorage 통합
      - [ ] 반려동물 프로필 데이터 LocalStorage 캐싱
      - [ ] 서버 데이터와 동기화
      - [ ] 오프라인 모드 지원 (읽기 전용)
    - [ ] 홈페이지 통합
      - [ ] 빠른 접근 메뉴에 "반려동물 건강" 아이콘 추가
      - [ ] 생애주기 이벤트 섹션에 반려동물 이벤트 통합
      - [ ] 알림 센터에 반려동물 백신 D-Day 알림 통합
  - [ ] UI/UX 고도화 기능 구현
