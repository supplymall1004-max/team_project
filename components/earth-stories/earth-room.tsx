/**
 * @file earth-room.tsx
 * @description 장고의 지구동화 스토리북 플레이어 메인 컴포넌트
 * 
 * GDWEB 기반 디자인 패턴 적용:
...
interface EarthRoomProps {
  videos: FoodStoryVideo[]
}

export function EarthRoom({ videos }: EarthRoomProps) {
  ...
  const handleRandomPlay = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      console.groupCollapsed("[EarthRoom] 랜덤 재생")
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 min-h-screen flex flex-col">
        ...
        <header className="text-center mt-4 mb-8 md:mb-12 animate-fade-in-up">
          ...
          >
            ...
            {displaySeason === "spring" && "🌸 꽃을 클릭하여 다양한 지구동화 이야기를 들어보세요."}
            {displaySeason === "summer" && "🌊 조개를 클릭하여 다양한 지구동화 이야기를 들어보세요."}
            {displaySeason === "autumn" && "🍂 낙엽을 클릭하여 다양한 지구동화 이야기를 들어보세요."}
            {displaySeason === "winter" && "❄️ 아이콘을 클릭하여 다양한 지구동화 이야기를 들어보세요."}
          </p>
        </header>
                ...
                {videos.map((video, index) => (
                  <div key={video.id} id={`gift-box-${video.id}`} className="scroll-mt-24">
                    <GiftBox
                      ...
                      onClick={() => {
                        console.groupCollapsed("[EarthRoom] 선물 상자 클릭")
