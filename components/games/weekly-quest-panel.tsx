/**
 * @file components/games/weekly-quest-panel.tsx
 * @description 주간 퀘스트 패널
 *
 * 주간 퀘스트 목록 및 진행 상황을 표시하는 UI 컴포넌트입니다.
 *
 * 주요 기능:
 * 1. 주간 퀘스트 목록 표시
 * 2. 진행 상황 표시
 ...
 * - @/lib/game/quest-system: WEEKLY_QUESTS, calculateQuestProgress
...
import { CheckCircle2, Trophy, RefreshCw, Calendar } from "lucide-react";
import { WEEKLY_QUESTS, calculateQuestProgress, type Quest } from "@/lib/game/quest-system";
...
export function WeeklyQuestPanel({ memberId, onQuestComplete }: WeeklyQuestPanelProps) {
  const [questProgresses, setQuestProgresses] = useState<Map<string, WeeklyQuestProgress>>(new Map());
  ...
  const loadQuestProgresses = useCallback(async () => {
    ...
    try {
      ...
      // const response = await fetch(`/api/game/weekly-quests?weekStart=${weekStart}`);
      // const data = await response.json();
      
      // 임시 데이터
      const progresses = new Map<string, WeeklyQuestProgress>();
  ...
  // 퀘스트 진행 업데이트
  const handleUpdateProgress = async (quest: Quest, newProgress: number) => {
    const weekStart = getWeekStart().toISOString().split("T")[0];
    const result = await completeQuest({
      questId: quest.id,
      progress: newProgress,
      questDate: weekStart, // 주간 퀘스트는 주 시작일 사용
    });
              ...
              {!progress.completed && (
                <div className="flex gap-2 mt-2">
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      handleUpdateProgress(quest, Math.min(progress.progress + 10, quest.target))
                    }
                  >
                    진행 업데이트
                  </Button>
