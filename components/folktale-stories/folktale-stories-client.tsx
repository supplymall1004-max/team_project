...
/**
 * @file folktale-stories-client.tsx
 * @description 전래동화 동영상 클라이언트 컴포넌트. 카드 그리드를 렌더링합니다.
 */
...
interface FolktaleStoriesClientProps {
  videos: FoodStoryVideo[];
  maxDisplay?: number;
  showViewAll?: boolean;
}

export function FolktaleStoriesClient({
  videos,
  maxDisplay,
  showViewAll = true
}: FolktaleStoriesClientProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 md:hidden scrollbar-hide">
        {displayVideos.map((video) => (
          <div key={video.id} className="min-w-[280px] max-w-[280px] flex-shrink-0">
            <FolktaleStoryCard video={video} layout="default" />
          </div>
        ))}
      </div>
      ...
      <div className="hidden grid-cols-1 gap-4 md:grid lg:grid-cols-3 justify-items-center">
        {displayVideos.map((video) => (
          <FolktaleStoryCard key={video.id} video={video} layout="default" />
        ))}
      </div>
      ...
      {showViewAll && maxDisplay && videos.length > maxDisplay && (
        <div className="flex justify-center pt-4">
          <Button variant="outline" asChild>
            <Link href="/folktale-stories">
              모든 전래동화 동영상 보기 ({videos.length}개)
            </Link>
          </Button>
        </div>
      )}
