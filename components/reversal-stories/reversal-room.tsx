/**
 * @file reversal-room.tsx
 * @description 장고의 반전동화 스토리북 플레이어 메인 컴포넌트
 * 
 * GDWEB 기반 디자인 패턴 적용:
...
interface ReversalRoomProps {
  videos: FoodStoryVideo[]
}

export function ReversalRoom({ videos }: ReversalRoomProps) {
  ...
  const handleRandomPlay = useCallback(() => {
    ...
    setIsLoading(true)
    setTimeout(() => {
      const otherVideos = videos.filter((video) => video.id !== selectedVideoId)
      const randomVideo =
        otherVideos.length > 0
          ? otherVideos[Math.floor(Math.random() * otherVideos.length)]
          : videos[Math.floor(Math.random() * videos.length)]

      console.groupCollapsed("[ReversalRoom] 랜덤 재생")
  ...
  const displaySeason = selectedSeason
    ...
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 min-h-screen flex flex-col">
        ...
        <header className="text-center mt-4 mb-8 md:mb-12 animate-fade-in-up">
          ...
          >
            전통 전래동화를 동화처럼 들려주는 이야기입니다.<br />
            {displaySeason === "spring" && "🌸 꽃을 클릭하여 다양한 반전동화 이야기를 들어보세요."}
            {displaySeason === "summer" && "🌊 조개를 클릭하여 다양한 반전동화 이야기를 들어보세요."}
            {displaySeason === "autumn" && "🍂 낙엽을 클릭하여 다양한 반전동화 이야기를 들어보세요."}
            {displaySeason === "winter" && "❄️ 아이콘을 클릭하여 다양한 반전동화 이야기를 들어보세요."}
          </p>
        </header>
                ...
                {videos.map((video, index) => (
                  <div key={video.id} id={`gift-box-${video.id}`} className="scroll-mt-24">
                    <GiftBox
                      ...
                      onClick={() => {
                        console.groupCollapsed("[ReversalRoom] 선물 상자 클릭")
