...
export async function HomeLanding() {
  ...
  const quickStartCards = [
    {
      title: allContent["quick-start-legacy"]?.content.title || "🎬 레거시 아카이브",
      description: allContent["quick-start-legacy"]?.content.description || "명인 인터뷰와 전통 조리법을 고화질로 감상하세요.",
      href: allContent["quick-start-legacy"]?.content.href || "/legacy",
    },
    {
      title: allContent["quick-start-recipe"]?.content.title || "📚 현대 레시피 북",
    ...
    {
      title: allContent["quick-start-diet"]?.content.title || "🤖 AI 맞춤 식단",
  ...
  return (
    <div className="space-y-4">
      ...
      <HeroSection
        ...
        description={allContent["hero-description"]?.content.text || "명인의 전통 레시피부터 AI 맞춤 식단까지, 세대와 세대를 넘나드는 요리 지식을 한 곳에서 경험하세요."}
        searchPlaceholder={allContent["hero-search-placeholder"]?.content.text || "레시피, 명인, 재료를 검색해보세요"}
