/**
 * @file actions/health/metrics.ts
 * @description 건강 메트릭스 Server Actions
 *
 * 사용자의 현재 건강 상태를 계산하여 반환합니다.
 * 기존 /api/health/metrics API Route를 Server Actions로 마이그레이션했습니다.
 *
 * 주요 기능:
 * 1. 건강 메트릭스 계산 및 조회 (getHealthMetrics)
 *
 * @dependencies
 * - @clerk/nextjs/server: auth
 * - @/lib/supabase/server: createClerkSupabaseClient
 * - @/lib/supabase/ensure-user: ensureSupabaseUser
 * - @/types/health-visualization: HealthMetrics
 ...
 */
export async function getHealthMetrics(): Promise<HealthMetricsResult> {
  try {
    console.group("[getHealthMetrics] 건강 메트릭스 조회 시작");
    ...
    console.log("[getHealthMetrics] 사용자 확인 및 동기화 시도...");
    const userData = await ensureSupabaseUser();

    if (!userData) {
      console.error("[getHealthMetrics] 사용자 동기화 실패:", {
        clerkId: userId,
      });
      console.groupEnd();
      throw new Error(
        "사용자 정보를 동기화할 수 없습니다. 잠시 후 다시 시도해주세요.",
      );
    }

    console.log("[getHealthMetrics] 사용자 확인 완료:", userData.id);
    ...
    if (profileError || !healthProfile) {
      console.log(
        "[getHealthMetrics] 건강 프로필이 없습니다. 기본값으로 계산합니다.",
      );
    ...
    console.error("[getHealthMetrics] 오류:", error);
    console.groupEnd();
    throw error instanceof Error
      ? error
      : new Error("건강 메트릭스 계산 중 오류가 발생했습니다.");
  }
}
...
async function calculateHealthMetrics(
  ...
  // 질병 위험도 계산
  const diseaseRiskScores = calculateDiseaseRiskScores(
    profile,
    bmi,
    nutritionBalance,
  );
  if (process.env.NODE_ENV === "development") {
    ...
    const diseaseCodes = Array.isArray(rawDiseases)
      ...
      : [];
    console.log(
      "[getHealthMetrics] 질병 코드/위험도 요약",
      { diseaseCodes, diseaseRiskScores },
    );
  }
...
function estimateBodyFatPercentage(profile: UserHealthProfile): number {
  ...
  const bmiAdjustment =
    Math.max(0, calculateBMI(weight_kg, height_cm) - 22) * 0.5;
...
function estimateMuscleMass(profile: UserHealthProfile): number {
  ...
  const baseMuscleRatio = gender === "male" ? 0.45 : 0.4;
 ...
 */
async function calculateCurrentNutritionBalance(
  profile: UserHealthProfile,
...
function calculateDiseaseRiskScores(
   ...
   */
  const hasDisease = (
    matchers: Array<(code: string) => boolean>,
  ): boolean => {
  ...
  const isHypertensionCode = (code: string) =>
    code.includes("hypertension") || code.includes("high_blood_pressure");
  const isHyperlipidemiaCode = (code: string) =>
    code.includes("hyperlipidemia") ||
    code.includes("high_cholesterol") ||
    code.includes("dyslipidemia");
  const isKidneyCode = (code: string) =>
    code.includes("kidney") || code === "ckd" || code.includes("renal");
  const isObesityCode = (code: string) =>
    code.includes("obesity") || code.includes("overweight");

  // "진단 보유"라면 위험도가 낮게 표시되지 않도록 하한(최소 위험도)을 적용
  const applyDiagnosedFloor = (
  ...
  Object.keys(scores).forEach((key) => {
    scores[key as keyof DiseaseRiskScores] = Math.min(
      100,
      Math.max(0, scores[key as keyof DiseaseRiskScores]),
    );
  });
...
function calculateOverallHealthScore(metrics: {
  ...
  // 질병 위험도 평균
  const avgDiseaseRisk =
    Object.values(metrics.diseaseRiskScores).reduce(
      (sum, risk) => sum + risk,
      0,
    ) / Object.values(metrics.diseaseRiskScores).length;
