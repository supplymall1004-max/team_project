...
/**
 * @file storage-test/page.tsx
 * @description Supabase Storage 연동을 테스트하는 페이지.
 * 파일 업로드/다운로드/삭제에 콘솔 로그를 남겨 디버깅을 용이하게 한다.
 */
...
export default function StorageTestPage() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    console.groupCollapsed("[StorageTest] 파일 목록 조회");
    try {
      setLoading(true);
      setError(null);
  ...
  const uploadFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.groupCollapsed("[StorageTest] 파일 업로드");
    try {
      setUploading(true);
      setError(null);
  ...
  const downloadFile = async (fileName: string) => {
    ...
    console.groupCollapsed("[StorageTest] 파일 다운로드");
    try {
      const filePath = `${user.id}/${fileName}`;
      const { data, error } = await supabase.storage
        .from("uploads")
  ...
  const deleteFile = async (fileName: string) => {
    ...
    console.groupCollapsed("[StorageTest] 파일 삭제");
    try {
      const filePath = `${user.id}/${fileName}`;
  ...
  if (!isLoaded) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <LoadingSpinner label="Clerk 세션을 불러오는 중..." />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="border rounded-lg">
        ...
        {loading ? (
          <LoadingSpinner label="파일 목록을 불러오는 중..." />
        ) : files.length === 0 ? (
