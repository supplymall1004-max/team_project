...
/**
 * @file auth-test/page.tsx
 * @description Clerk + Supabase 연동 상태를 검증하는 테스트 페이지.
 *
 * 핵심 로직마다 콘솔 로그를 남겨 상태 추적이 가능하도록 구성.
 */
...
export default function AuthTestPage() {
  ...
  const testConnection = useCallback(async () => {
    console.groupCollapsed("[AuthTest] Supabase 연결 테스트");
    try {
  ...
  const fetchOrCreateUser = useCallback(async () => {
    ...
    console.groupCollapsed("[AuthTest] 사용자 데이터 동기화");
    try {
      setLoading(true);
      setError(null);
  ...
  const updateName = async () => {
    ...
    console.groupCollapsed("[AuthTest] 이름 업데이트");
    try {
      setError(null);
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="border rounded-lg">
        ...
        <div className="p-6">
          {loading ? (
            <LoadingSpinner label="데이터를 불러오는 중입니다" size="sm" />
          ) : userData ? (
