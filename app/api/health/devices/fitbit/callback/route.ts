/**
 * @file app/api/health/devices/fitbit/callback/route.ts
 * @description Fitbit OAuth 콜백 처리
 */
...
export async function GET(request: NextRequest) {
  try {
    console.group('[API] GET /api/health/devices/fitbit/callback');
    ...
    if (!clientId || !clientSecret) {
      console.error('❌ Fitbit 설정이 완료되지 않았습니다');
      console.groupEnd();
      return NextResponse.redirect(
    ...
    console.log('✅ Fitbit 연동 완료:', dataSource.id);
    console.groupEnd();

    // 성공 페이지로 리다이렉트
    return NextResponse.redirect(
      new URL('/health/profile?tab=devices&connected=fitbit', request.url)
    );
  } catch (error) {
