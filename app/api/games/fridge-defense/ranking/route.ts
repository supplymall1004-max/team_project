...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. 점수 데이터만 먼저 조회 (game_type: 'fridge_defense')
    const { data: rankingData, error: rankingError } = await supabase
      .from('fridge_guardian_scores')
      .select('user_id, score, played_at')
      .eq('game_type', 'fridge_defense')
      .order('score', { ascending: false })
      .limit(limit * 5); // 중복 제거를 위해 더 많이 가져옴

    if (rankingError) {
      console.error('[FridgeDefense API] 랭킹 조회 실패:', rankingError);
      return NextResponse.json(
    ...
    if (usersError) {
      console.warn('[FridgeDefense API] 사용자 정보 조회 실패:', usersError);
    }
    ...
    console.error('[FridgeDefense API] 랭킹 조회 예외:', error);
    return NextResponse.json(
