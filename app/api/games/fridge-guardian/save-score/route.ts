/**
 * @file app/api/games/fridge-guardian/save-score/route.ts
 * @description 냉장고 파수꾼 게임 점수 저장 API
 * 
...
import type { GameStats } from '@/types/game/fridge-guardian';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userError || !userData || !userData.id) {
      console.error('[FridgeGuardian API] 사용자 조회 실패:', userError);
      return NextResponse.json(
    ...
    console.log('[FridgeGuardian API] 점수 저장 시도:', {
    ...
    if (error) {
      console.error('[FridgeGuardian API] 점수 저장 실패:', {
        code: error.code,
    ...
    console.log('[FridgeGuardian API] 점수 저장 성공:', data.id);
    return NextResponse.json({ 
      success: true, 
      data: {
        id: data.id,
    ...
    console.error('[FridgeGuardian API] 점수 저장 예외:', error);
    return NextResponse.json(
